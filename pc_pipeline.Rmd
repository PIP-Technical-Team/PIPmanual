...
chapter, **pc pipeline**--is the technical procedure to calculate the
...
the section above, the scripts `source('R/_common.R')` loads longer functions.
Yet, keep in mind that the `'R/_common.R'` was used in a previous version of the
pc pipeline before `{targets}` was implemented. Now, most of the function in
`'R/_common.R'` are included in the `{pipdm}` package.

### Step 2: prepare data {#pipe-prepare-data .unnumbered}

This section used to longer in previous versions of the pc pipeline because
...
create the target `aux_dir`, using the file paths in column `auxfiles` of
`aux_tb` to let `{targets}` know that we will have objects that are loaded from
...
Most of the targets created in the pc pipeline are **branch** targets. This is
...
single data set or result were a **stem** target, it would be not only
...
add the argument `format = "file"` to let `{targets}` know that it is input
data. In step B we load the cache inventory file into target `cache_inventory`
...
function `tar_files()`, creating thus a new target, `cache_dir`. FInally, we
create **branch** target `cache` with all the cache files by loading each file.
To do this iteratively, we parse the `cache_dir` target to the `path` argument
of the function `fst::read_fst()` and to the `pattern = map()` argument of the
`tar_target()` function. Finally, we need to specify that the output of the
iteration is stored as a list, using the argument `iteration = "list"`.

The basic logic of **branch** targets is that the vector or list to iterate
though should be added in the argument of the function and the `pattern = map()`
argument of the `tar_target()` function. it is very similar to `purrr::map()`
...
the cache files used to be created in [Step 2: prepare data] rather than here.
...
pipeline, by following the instructions in [Executing the \_targets.R file].

The first target, `pipeline_inventory` is the pip inventory dataset filtered to
...
the information for IDN, 2015. [In case you need to update specific cache files,
you have to do add the proper filtering condition in there]{style="color:red"}.

In the second target, `status_cache_files_creation`, you will create the cache
files, but the returning value of the function `pipdm::create_cache_file()` is
...
that function `pipdm::create_cache_file()` requires the the CPI and PPP
auxiliary data. This is so because the cache file has an additional variable,
`welfare_ppp`, for deflated welfare aggregate to daily 2011 PPP values. FInally,
and more importantly, argument `force = TRUE` ensures that even if the cache
...
are testing, and only need a few surveys with the new features.
...
`wbpip::md_compute_dist_stats()` and make sure the new output does not mess up
...
good example why, this structure might need be improved.
...
you can execute the whole thing confidently.

In order to execute the whole pipeline, you only need to type directive
...
Debugging in targets is not easy. Yet, there are two ways to do it. On the one
hand, you can take a look at the chapter
[Debugging](https://books.ropensci.org/targets/debugging.html) in the Targets
Manual. It provides clear instruction on how to do it. On the other hand, you
can see if the following ideas work.
