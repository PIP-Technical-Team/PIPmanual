...
According to the description in the Stata repository [worldbank/primus](https://github.com/worldbank/primus),
...
As such, **PRIMUS** is the platform used by the GPID team to approve the adoption of new survey data into the PIP system.
...
PRIMUS can be accessed through https://primusuiprod.ase.worldbank.org/. As long as you're connected to the intranet it should work fine. However, if you have any issues connecting to the platform, please send an email to [Minh Cong Nguyen](mailto:mnguyen3@worldbank.org), requesting access.

Each database uploaded into PRIMUS gets a unique **transaction ID**. This ID is important because it is not unique to a dataset but unique to the transaction (or vintage of the data). That is, if one particular dataset is uploaded more than once, it will get two different transaction IDs. When communicating with the Poverty GP, you should refer to the transaction ID rather than the survey (or at least both) because the same country/year, you might have two different transactions.

### Stata command {.unnumbered}

The Poverty GP maintains the Stata repository [worldbank/primus](https://github.com/worldbank/primus) from which you can download the command `primus`. Right now, this is the official place from which you can access this command. From now on, each time we refer to the command, we use `primus`, whereas when we refer to the website, we use PRIMUS.

Please, make sure you have it properly installed in your computer, by following the instruction section \@ref(stata-github). Basically, you need to install first the [github](https://github.com/haghish/github) Stata command by [E. F. Haghish](https://github.com/haghish)
...
The `primus` command is maintained by the Poverty GP, so we have no control over modifications or improvements. The best you can do in case you need to fix or modify something in this command is to fork the repository, clone the forked repo into your computer, check out a new branch, make any modification, and generate a pull request to the master branch of the original repository. Once you have done that, make sure to send an email with your suggestions for improvement to [Ani Rudra Silwal](mailto:asilwal@worldbank.org), copying to the D4G Central Team (Nobuo Yoshida and Minh Cong Nguyen).
...
The real first step to check the quality of the recently uploaded data into PRIMUS is to download the basic estimates for each data point and compare them with our own. New estimates are uploaded into `primus` mainly before the Annual Meetings (AM) in the **Fall** season, or the **Spring** Meetings (SM) in March/April. For the most recent rounds, we have created a repository called **Test-PIP-Data** in the GPID-WB GIT organization where we've been working and storing this comparison:

1.  Clone the repository [**h**ttps://github.com/GPID-WB/Test-PIP-Data](https://github.com/GPID-WB/Test-PIP-Data)
2.  Go to the folder 📁 **Check_PRIMUS_Data**. Inside, you will notice that there are individual sub-folders for each data release period.
3.  In your new branch, create a [new folder]{.underline} for data release on which you're working (i.e Spring 2025)

Note that the `primus` stata command allows to download the estimates for each transaction, but only one by one. There are *two ways* to download and stack all new estimates into one single file:

**A). with `pcn` command:** the `pcn` command downloads all the estimates of pending transactions for us and properly stores them in the P drive, 📁 `p:\01.PovcalNet\03.QA\02.PRIMUS\pending\....`. If you have previously installed `pcn`, type:

``` stata
pcn primus pending, down(estimates)
```

In addition, `pcn` checks the date for which you're downloading the estimates and keeps only those transactions that have been uploaded for the next spring or annual-meetings release. For instance, assume that if today is 2023-11-20, you want to see the estimates of pending transactions in PRIMUS. Since annual meetings take place around September, `pcn` assumes you are interested in the estimates for the Spring-meetings release, around March next year. Thus, it will filter `primus` results keeping only those transactions that were uploaded starting November 2023. It is likely that the PRIMUS system has not been opened for uploading new data in November, as it usually opens around December and July. Thus, it is likely that you will find and error saying `There is no pending data in PRIMUS for the combination of country/years selected`. So consider the timing before running that command. You can load the recently-downloaded estimates by typing:

``` stata
pcn primus pending, load(estimates)
```

Using `pcn` does the job but makes the process dependent on that command as well as the P drive. The following option (B) is what we've been using in most recent rounds, keeping all files in the cloned repository:

**B). with available code in repository:**

Going back to the repository you cloned (Test-PIP-Data/Check_PRIMUS_Data), look inside the folder for the previous update. You will find a do.file named **`Compare PRIMUS PENDING vs PIP estimates.do`** Create a copy of this file and paste it into your newly created folder. Update the necessary paths and dates. This do.file conducts the comparison exercise in 3 overall steps:

1.  Stacks all **PRIMUS** pending cases (individual transationIDs), cleans & reshapes estimates and saves them in file (`primus_estimates_[DATE].dta`)
2.  Loads current **PIP** estimates and saves them in file (`pip_estimates_[DATE].dta`)
3.  Merges both data sets, takes differences & saves them in file (`Comparison_primus_vs_pip.dta`).

Note that we don't have to compare all estimates, headcount, gini and mean will be enough.

::: callout-important
PRIMUS **does not** have a clear variable to signal **welfare type** (I/C)!
:::

As you know, several countries in PIP (i.e. Poland, Hungry, Philippines, etc) have **dual welfare** in the same year. Because PRIMUS lacks a clear variable to flag whether the new estimate is based income or consumption, this needs to be **checked "manually", case by case** after merge. For this, it is advisable to save a copy of the final file into **Excel** to finalize the comparison.

Note that the new PRIMUS cases can either be 1) an update of an existing estimate or 2) a newly added year in the series. Use the variable `_merge` to flag either case. Finally, flag (color code) the differences that are unusually large, and make sure that countries with dual welfare (if any), didn't get merged erroneously.
...
For some unknown reason, the PRIMUS system did not accept the approval of some transactions. If this happens again, you need to talk to [Minh Cong Nguyen](mailto:mnguyen3@worldbank.org), so he can do the approval manually.
