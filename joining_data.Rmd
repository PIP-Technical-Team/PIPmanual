...
variable in auxiliary data used to merge any welfare data, `data_level`.
...
CHN <-  pipload::pip_load_data(country = ccode, 
                              year    = 2015)
domains <- 
  pfw[ country_code == ccode & surveyid_year == 2015, 
       .(cpi = unique(cpi_domain_var), 
         ppp = unique(ppp_domain_var))][]



byvars <- glue::glue("{domains[['cpi']]} = data_level")
# byvars <- parse(text = byvars)
debugonce(joyn::merge)
joyn::merge(CHN, cpi, 
            by = byvars)
