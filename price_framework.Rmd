...
The table provides a short description of each variable of the Price FrameWork 
data frame. We provide additional information for variable.  
...
There is also difference between `year` and `surveyid_year`. For most of the data points, there should be no difference in between year and surveyid_year. However, in some extreme cases (not available at the moment), year could be different due to the reporting criteria from the NSO. For our purpose, we use `rep_year` as the reporting year in our system for integer ones, and `ref_year` for decimal reporting purpose.
